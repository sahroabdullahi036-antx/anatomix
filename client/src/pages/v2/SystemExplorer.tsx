...
export default function SystemExplorer() {
  ...
  const currentColor = currentSystem ? SYSTEM_COLORS[currentSystem.id] : "#596e60";
  ...
  const styles = {
    page: { minHeight: "100vh", backgroundColor: "#8b4f58", fontFamily: "'Inter','Plus Jakarta Sans',sans-serif", padding: "0" } as React.CSSProperties,
    header: { backgroundColor: "rgba(0,0,0,0.2)", padding: "14px 24px", display: "flex", alignItems: "center", gap: "12px", borderBottom: "1px solid rgba(252,250,247,0.1)" } as React.CSSProperties,
    body: { maxWidth: "820px", margin: "0 auto", padding: "32px 24px" } as React.CSSProperties,
    card: { backgroundColor: "rgba(0,0,0,0.25)", borderRadius: "14px", padding: "24px", marginBottom: "20px", border: "1px solid rgba(252,250,247,0.1)" } as React.CSSProperties,
    ...
    back: { backgroundColor: "rgba(252,250,247,0.12)", color: "#fcfaf7", border: "1px solid rgba(252,250,247,0.2)", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontFamily: "inherit", fontSize: "0.9rem" } as React.CSSProperties,
    label: { color: "rgba(252,250,247,0.5)", fontSize: "0.75rem", fontWeight: "600" as const, textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: "6px" },
    value: { color: "#fcfaf7", fontSize: "1rem", fontWeight: "600" as const, marginBottom: "14px" },
    def: { color: "rgba(252,250,247,0.8)", fontSize: "0.9rem", lineHeight: 1.6 },
  };
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "14px" }}>
            {SYSTEMS.map(sys => (
              <button key={sys.id} onClick={() => selectSystem(sys)} style={styles.btn(SYSTEM_COLORS[sys.id])}>
                <div style={{ fontSize: "28px", marginBottom: "8px" }}>{sys.emoji}</div>
                <div style={{ fontSize: "1rem", fontWeight: "700" }}>{sys.casualName}</div>
                <div style={{ fontSize: "0.75rem", opacity: 0.7, marginTop: "4px" }}>{sys.officialName}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={styles.page}>
        ...
        <div style={styles.body}>
          <div style={styles.card}>
            ...
            <div style={{ color: "#fcfaf7", fontSize: "1.4rem", fontWeight: "800", marginBottom: "4px" }}>{currentSystem.officialName}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "12px" }}>
            {currentSystem.structures.map(s => (
              <button key={s.id} onClick={() => selectStructure(s)} style={styles.btn(currentColor)}>
                ...
                <div style={{ fontSize: "0.75rem", opacity: 0.7, marginTop: "4px" }}>{s.officialName}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {selected && (
        <div style={styles.body}>
          ...
          {selected.children && selected.children.length > 0 && (
            <>
              <h2 style={{ color: "rgba(252,250,247,0.7)", fontSize: "0.85rem", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "12px" }}>Sub-Structures</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "12px" }}>
                {selected.children.map(child => (
                  <button key={child.id} onClick={() => selectChild(child)} style={styles.btn(currentColor)}>
                    ...
                    <div style={{ fontSize: "0.75rem", opacity: 0.7, marginTop: "4px" }}>{child.officialName}</div>
                  </button>
                ))}
              </div>
            </>
